...
plot.write_text('\n'.join(plines)+'\n')
# Freeze guard: only the measured column may differ from the original plan.
frozen=json.loads((C/'plot_design_frozen.json').read_text())
for line in plines:
    m=re.match(r'^\|\s*(\d+)\s*\|',line)
    if m:
        cells=[s.strip() for s in line.split('|')[1:-1]];r=frozen['chapters'][int(m.group(1))-1]
        assert cells[1:5]==[r['title'],r['part'],r['kind'],str(r['design_chars'])]
        assert list(map(int,re.findall(r'\d+',cells[-1])))==r['facts']
