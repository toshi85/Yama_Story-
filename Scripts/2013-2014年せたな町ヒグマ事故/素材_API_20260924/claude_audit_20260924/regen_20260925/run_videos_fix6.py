#!/usr/bin/env python3
"""動画の残り（video_plan_fix6.json）を、元の静止画ができたものから fal_video_queue.py（H3）で作る（2026-09-26）。
...
plan = json.loads((HERE / "video_plan_fix6.json").read_text(encoding="utf-8"))
...
while time.time() < end:
    ...
    try:
        ok = set((HERE / ".imagegen" / "video_ok.txt").read_text(encoding="utf-8").split())
    except OSError:
        ok = set()
    ready = [p for p in todo if p["id"] in ok and (ROOT / p["image"]).exists() and not (OUT / f"{p['id']}.request.json").exists()]
    by_md = {}
    for p in ready:
    ...
    for md, items in by_md.items():
        ...
        r = subprocess.run([PY, "-u", QUEUE, str(HERE / "video_plan_fix6.json"), "--root", str(ROOT), "--out", str(OUT),
