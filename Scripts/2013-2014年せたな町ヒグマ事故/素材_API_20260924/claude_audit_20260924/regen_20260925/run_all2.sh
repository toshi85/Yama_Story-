...
touch .imagegen/skip_ids.txt .imagegen/defer_ids.txt .imagegen/gave_up.txt
reset_tabs() {
  $PY - <<'PY'
...
skip = set(pathlib.Path('.imagegen/skip_ids.txt').read_text().split()) | set(pathlib.Path('.imagegen/defer_ids.txt').read_text().split()) | set(pathlib.Path('.imagegen/gave_up.txt').read_text().split())
...
for round in 1 2 3 4; do
  : > .imagegen/defer_ids.txt; : > .imagegen/gave_up.txt   # 周のはじめに後回し・外したカットを戻す
  for fix in ${=FIXES:-Fix5c Fix5b Fix5a Fix4 Fix2 Fix3}; do
    ...
    while [ "$(left)" -gt 0 ] && [ $stall -lt 6 ]; do
      ...
      reset_tabs
      ex=$(cat .imagegen/skip_ids.txt .imagegen/defer_ids.txt .imagegen/gave_up.txt | tr '\n' ',' | sed 's/,$//')
    ...
    echo "===== $(date +%H:%M) 周${round} ${fix} おわり（残り $(left)枚・後回し $(cat .imagegen/defer_ids.txt .imagegen/gave_up.txt | sort -u | grep -c .)件）"
  done
  total=$($PY -c "
