...
sid=int(sys.argv[1]); work=CHECK/'preview_work_2'/str(sid);work.mkdir(parents=True,exist_ok=True)
# Preserve any prior preview outputs and lock files; use a fresh output directory.
outdir=CHECK/('preview_2_adjusted' if sid==23 else 'preview_2');outdir.mkdir(exist_ok=True)
def run(cmd,**kwargs):
