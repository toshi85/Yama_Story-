...
ChatGPTの「リクエストが多すぎます」ダイアログの「了解」は検知時に押す。待機中も15秒ごとに確認し、再表示されれば閉じる。通知を閉じる操作と再送を分け、「数分待つ」の表示に対して再送は3分待つ。期限前には再送しない。
...
制限判定は `driver.js` の `readLimitEvidence()` が返す現在の明示的な表示文・種類・観測時刻を使う。要求文と履歴タイトルを除外する。過去ログのキーワードや曖昧な待ち案内で判定しない。
`generation_status.json` の `evidence` は判定根拠、`retry_at` はシステムの次回再試行予定、`until` は画面から読み取れた解除目安。互いを同じ時刻として表示しない。
明示的な表示が現在なければ、過去の待機理由は `scheduled_wait` として扱い、現在の上限と断定しない。
