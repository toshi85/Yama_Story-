#!/usr/bin/env python3
"""較正（2026-09-14 戸沢村）: 旧版10件中9件を FAIL、修正版20件中17〜19件を PASS。
見逃し例=116（主語の欠落）、誤FAIL例=050（抽象的な行為）。判定は揺れるので、FAIL は人が読む前提。
...
1. 物差しは1本だけです。ナレーションで言っている主語・主題（人物・物・出来事）が、プロンプトの絵に見えるかを判定してください。「反応」「痕跡」「書類」「風景」で主題を代替していたら FAIL です。
   ...
   まずナレーションに出る人物・動物を全員書き出してください（例: 「奥さんの近くにクマがいて、近づけません」→ 奥さん・クマ・（近づけない）夫 の3者）。その全員がプロンプトに登場しなければ FAIL とし、missing に欠けた者を書いてください。
   遺体の傷・損傷がナレーションの主題のとき、プロンプトが後ろ姿・無傷の姿だけで、「編集で赤いハイライトを重ねる」「指示線を足す」と編集者指示で代替していたら FAIL です。傷は破れた服・欠損の陰影として絵に描くのが方針です。
2. ただし次は PASS です。
   ...
   ⑦ 主題が「行為の結果・傾向・比較」（許可を取る／頭を狙う傾向／体格差）の場合は、主語（人物・クマ）が主題に関わる姿（書類を持つ／2頭を並べる）で描かれていれば PASS。図解・矢印・数値は編集者指示に任せる
      ⑦は抽象的な行為・傾向・比較に限ります。負傷・遺体・襲撃・捕獲など目に見える出来事には適用しません。
3. 遺体・負傷・捕獲したクマ・襲撃の瞬間は、カートゥンで描くのが方針です。描かずに逃げていたら FAIL にしてください。
4. 次の食い違いも FAIL です（2026-09-26 せたな町で本人が指摘した型）。missing に食い違いを1行で書いてください。
   ① 年齢: 場面の人物（山菜採り・証言者・被害者）の年齢がナレーション・シーン行と合わない。年齢を書かずに若く描かれうる人物も FAIL
   ② 役割: 依頼する側と引き受ける側、呼びかける側と聞く側を取り違えている（例: 町が専門家に依頼する場面で、専門家が提案している）
   ③ 時系列: 駆除・捕獲の後なのにクマが生きて吠えている／打ち切り・撤収なのに山へ向かっている
   ④ 表情: ナレーション・シーン行に泣く記述が無いのに泣いている
   ⑤ 絵柄: キャラ画像の背景に実写のクマがいる（クマはキャラで描く方針）
5. 評価語や提案は書かないでください。JSON 以外を出力しないでください。
