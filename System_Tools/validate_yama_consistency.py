...
def main():
    ...
    #
    #   戸沢村で実際に起きた事故:
    #     §9  「一人目は背後から。二人目は前から。」   ← 米田 p.206
    #     §19 「三件とも、クマは身を伏せて、後ろから」 ← 米田 p.208
    #   同じ本の中で記述が割れているのに、台本は両方を断定で書いてしまった。
    #   視聴者から見れば、ただの矛盾。
    #
    #   ルール: 対立する主張の両方が台本にあるのは構わない。
    #           ただし「食い違っている」と明示していなければ FAIL。
    #           資料が割れているなら、割れているまま出すのが台本の作法。
    # ------------------------------------------------------------------
    #   1回目の実装は、この事故を止められなかった。「食い違いを明示しているか」を
    #   台本全体で探したため、§23 にある別件の「食い違ったままでした」（三人目の性別）
    #   を拾って PASS にしてしまった。明示は、矛盾している当の章になければ意味がない。
    narr = []          # [(章番号, 本文)]
    cur_no = 0
    for l in open(mp, encoding="utf-8"):
        ...
        if m:
            ...
            narr.append((cur_no, re.sub(r"\s*<!--.*?-->", "", l.split(":", 1)[1]).strip()))
    by_ch = {}
    for n, t in narr:
        by_ch.setdefault(n, []).append(t)

    # 対立語ペア: (A側の言い方, B側の言い方, 何の対立か)
    CONTRADICT = [
        (r"背後から|後ろから|後方から|潜行して",
         r"前から|前方から|正面から",         "襲われた方向"),
        (r"満腹|空腹では(なかった|ありません)",
         r"空腹|飢え(て|によ)",               "空腹か満腹か"),
        (r"同じ一頭|同一個体|一頭による",
         r"複数(の|頭)|別の個体|二頭",         "加害個体の数"),
        (r"即死|その場で(亡く|息を)",
         r"病院へ運ば|搬送され",               "亡くなった場所"),
    ]
    # 「割れている」と明示していれば、それは矛盾ではなく誠実な提示
    DISCLOSED = r"食い違|割れて|一致しま|異なって|とする一方|別の(ページ|箇所)|断定でき|分かって(いま|おり)ませ"

    # 全称表現＝「三件とも」「三人とも」「全員が」。個別章の記述を丸ごと上書きする言い方
    ALL_OF = r"(?<![十百])(三件|3件|三人)とも|全員(が|は)|いずれも"

    #   2回目の実装も止められなかった。§9「一人目は背後から。二人目は前から。」が
    #   両側の語を含むため、章ごと候補から外れてしまった。
    #   矛盾を作るのは「全称の断定」のほう。そこを起点に、逆を言う章を探す。
    seen = set()
    for pa, pb, what in CONTRADICT:
        for pat_all, pat_rev in ((pa, pb), (pb, pa)):
            for n, t in narr:
                # 起点は「三件とも〜後ろから」のような、個別章を上書きする全称の断定
                if not (re.search(pat_all, t) and re.search(ALL_OF, t)):
                    continue
                if re.search(pat_rev, t):
                    continue            # 同じ行に両方＝対比。矛盾ではない
                for n2, t2 in narr:
                    if n2 == n or not re.search(pat_rev, t2):
                        continue
                    # 明示は「矛盾している当の章」にあること。別の場所にあっても無意味
                    if any(re.search(DISCLOSED, x)
                           for x in by_ch.get(n, []) + by_ch.get(n2, [])):
                        continue
                    key = (what, n, n2)
                    if key in seen:
                        continue
