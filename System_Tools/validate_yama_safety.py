...
BANNED_WORDS = {
    ...
    # 台本が本の要約だと視聴者に分かってしまう。
    # 🚨 禁止するのは**書籍の著者名**と**本そのものを指す語**だけ。
...
_AUTHORS = os.path.join(os.path.dirname(os.path.abspath(__file__)), "book_authors.txt")
if os.path.exists(_AUTHORS):
    for _line in open(_AUTHORS, encoding="utf-8"):
        _name = _line.split("#")[0].strip()
        if _name:
