...
def validate_narrative_tone(file_path):
    ...
    #   前: 1頭だと考える根拠は、三つ。／ 犠牲になった方の傷が似ていること。／ …
    #   後: 1頭だと考える根拠としては、／ 犠牲になった方の傷が似ていること。／ また、…／ そして…
    #   個数を先に言うと、視聴者は数を数えながら聞くことになり、中身から気がそれる。
    #   「〜としては、」で受けて「また、」「そして」で継ぐ。
    #
    #   🚨 FAIL にしない。出荷済み4本に4件ある形（星野道夫・大千軒岳・羅臼岳・風不死岳）。
    #   ⚠️ 「最も奥まった場所のひとつです」のような「〜のひとつ」は除外している（2026-09-03 較正）。
    _CNT = _re3.compile(r"(?:は|が)、?(?<![のな])"
                        r"(?:一つ|二つ|三つ|四つ|五つ|ひとつ|ふたつ|みっつ|[１-９1-9]つ)"
                        r"(?:[。、]|$|あります|ありました|挙げ|です[。、]?$)")
    for _no, _l in _nl:
    ...
    #   前: ナレーター: 小林室長は、こう話しています。
    #       ナレーター: 「事件の前か後かは不明ですが……」
    #   後: ナレーター: 小林室長は、「事件の前か後かは不明ですが……」
    #       ナレーター: 「……ようです」と話しています。
    #   「こう話しています。」で一度切ると、視聴者は誰の言葉かを覚えたまま待たされる。
    #   発言を先に流し、最後に「と話しています」で閉じる。
    #
    #   🚨 FAIL にしない。出荷済み6本に12件ある形で、遡って直す対象ではない（2026-09-03 実測）。
    _CUE2 = _re3.compile(r"(こう|そう|次のように)(話して|語って|書いて|記して|述べて|残して|コメントして)います。?$")
    for _no, _l in _nl:
        _t = _re3.sub(r"\s*<!--.*?-->", "", _l).split(":", 1)[1].strip()
        if _CUE2.search(_t):
            warnings.append(
