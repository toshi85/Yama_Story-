...
- [ ] **人物が動く・反応するカットには、編集者指示とは別の行で `→セリフ「…」` を付ける。転換・強調の一文は「黒背景に白テキスト＋強調語1つ赤字」にする**（2026-09-13 本人追記117カットから抽出。型と数字 → `.claude/skills/yama-asset-prompt/references/セリフと画面文字の型.md`）
...
| **本人チェックの定番（40-44・WARNのみ）** | **セリフの方言語尾**／**キャラアニメにセリフなし**／**14字超のテロップ**／**読点で割ったナレーション**／**テロップ抑制指示** |

> 40-44は2026-09-15追加（戸沢村の本人チェック125か所から抽出。すべてWARNで、終了コードは変えない）。
